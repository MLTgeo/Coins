...
object Simulation extends App {
  ...
  val towns = args(0) //"/tmp/Towns.txt"
  val resultsDir = args(1) //"/tmp/Results/"
  ...
  val model =
    new Coins2Cities {
      def exchangeLaw(value: Int, rng: Random) = rng.nextDouble /*value match {
                                                                 case 0 => gaussian(0.5, 1, rng)
                                                                 case 1 => gaussian(0.2, 1, rng)
                                                                 }*/
