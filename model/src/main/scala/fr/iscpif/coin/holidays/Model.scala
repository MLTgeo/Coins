            ...
            rng.shuffle(commuterDestinations).zipWithIndex.map {
              case (destination, i) =>
                new Agent(
                  source,
                  destination,
                  if (i < nbCommuterTourists) touristicCities(i % touristicCities.size) else destination,
            ...
            (0 until source.population - mobileAgents.size).map(
              i =>
                new Agent(
                  source,
                  source,
                  if (i < nbStaticTourists) touristicCities(i % touristicCities.size) else source,
                  buildWallet(source, countries.size))
            )
